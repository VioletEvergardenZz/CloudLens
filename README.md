...
它不是一个复杂的一体化运维平台，而是先把个人和小团队最常用的多云资源可见性跑顺：接入云账号，看清 ECS/RDS 资源，快速判断实例状态、计费到期和监控指标。告警、AI、知识库和文件入云仍保留为辅助能力，后续会围绕资源异常排查继续收口。
...
| 配置 | `.env` 优先，`config.yaml` 作为文件入云等旧配置补充 |
...
| [开发指南](./docs/02-开发运维/开发指南.md) | 本地开发、运维和回归口径 |
