...
<p align="center">
  ...
  <img alt="华为云 ECS/RDS" src="https://img.shields.io/badge/Huawei%20Cloud-ECS%20%7C%20RDS-C7000B?logo=huawei&logoColor=white">
...
| 华为云 RDS | 已落地 | 实例列表、空间用量、连接端点和 CES 性能指标概览 |
| 资源快照降级 | 已落地 | ECS/RDS 列表成功同步后保存快照，云 API 临时失败时返回旧快照兜底 |
| 运维体检 | 已落地 | 汇总账号诊断、风险中心、轻量巡检报告和运行检查 |
| 本地 SQLite | 默认使用 | 不需要单独启动数据库，适合轻量部署 |
| 告警 | 辅助能力 | 已支持告警事件人工处置闭环，后续围绕资源异常排查继续增强 |
...
不接真实云账号时可以直接启动。需要接云账号或修改端口时，再执行 `cp .env.example .env` 并编辑 `.env`。
...
Docker Compose 默认使用命名卷持久化配置、日志和 SQLite 数据，不要求本机预先创建 `/opt/cloudlens` 目录。
...
Docker Compose 端口和云账号配置见：[.env.example](./.env.example)。后端完整环境变量示例见：[cloudlens-backend/.env.example](./cloudlens-backend/.env.example)。
...
| `GET /api/cloud/huawei/rds/instances` | 华为云 RDS 实例列表 |
| `GET /api/cloud/huawei/rds/overview` | 华为云 RDS 性能概览 |
| `GET /api/cloud/snapshots` | 最近一次 ECS/RDS 资源快照摘要 |
| `GET /api/resources` | 基于快照生成的统一资源索引 |
| `GET /api/resources/{id}` | 统一资源详情 |
| `GET /api/resources/{id}/metrics` | 按统一资源 ID 查询 ECS/RDS 指标 |
| `GET /api/inspection/risks` | 基于统一资源索引生成巡检风险 |
| `GET /api/k8s/overview` | Kubernetes 只读巡检总览，支持 kind/kubeconfig |
| `GET /api/k8s/node-links` | K8s Node 与 ECS 资源关联结果 |
| `GET /api/cloud/diagnostics` | 云账号接入诊断和快照覆盖 |
| `GET /api/cloud/risks` | 基于快照识别到期、公网暴露、RDS 存储水位等风险 |
| `GET /api/cloud/inspection-report` | 轻量巡检报告，支持 `?format=markdown` |
| `GET /api/runtime/checks` | 部署与运行健康检查 |
...
- 当前真正落地的云厂商能力包含阿里云 ECS/RDS 和华为云 ECS/RDS。
- 前端“运维体检”页用于统一查看账号诊断、风险中心、轻量巡检报告和运行检查。
