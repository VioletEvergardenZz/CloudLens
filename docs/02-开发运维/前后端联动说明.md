...
**前端（console-frontend）**
- 展示平台总览、目录树 / 文件列表 / 上传记录 / 队列趋势 / Tail 日志 / AI 总结。
- 通过 API 拉取仪表盘数据，并触发上传与配置更新操作。
- 告警控制台展示风险概览、告警列表与告警配置。
- 域名接入控制台展示入口探测结果、候选健康接口与待确认事项。
- 事件工作台与系统台账页作为统一平台原型页，优先承接现有接口与后续纳管事实源。
...
```mermaid
sequenceDiagram
  participant UI as RegistryConsole
  participant API as API Server
  participant PROBE as DomainProber

  UI->>API: POST /api/registry/domain-probe {domain}
  API->>PROBE: Probe(domain)
  PROBE-->>API: DNS / HTTP / HTTPS / TLS / healthCandidates / pendingItems
  API-->>UI: { ok, result }
```

补充说明：
- 输入支持域名或带协议的入口地址，后端会统一归一到域名目标。
- 当前接口默认拒绝 `IP / localhost / 仅解析到内网地址`，避免探测接口被误用为匿名 SSRF 入口。
- 页面展示重点不是“能不能访问”，而是“建议接入类型 + 下一步待确认项”。
...
**13) 域名接入探测**
- `POST /api/registry/domain-probe`
- 调试兼容：`GET /api/registry/domain-probe?domain=...`
- 请求体：
  ```json
  {
    "domain": "mch.58victory.com"
  }
  ```
- 返回：`{ ok, result }`
- `result` 结构包含：
  ...
  - `tls`
  - `healthCandidates`
  - `suggestedAccessType / suggestedAccessTypeLabel`
  - `pendingItems`
