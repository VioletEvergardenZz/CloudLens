...
- `gwf-backend` 默认会挂载 `/var/run/docker.sock`，因此可直接监控宿主机 Docker 容器日志。
- 推荐把 `alert_log_paths` 配成 Docker 源，而不是在服务器里手工翻找业务日志目录。
- 常用写法：`docker://service/api?project=myapp`、`docker://container/my-api-1`。
- 容器日志会镜像到 `logs/alert-sources/docker/`，后续 AI 分析与全文检索继续走原有接口。
...
- 当 `alert_log_paths` 使用 `docker://...` 时，可直接分析 `logs/alert-sources/docker/` 下的镜像日志文件。
...
## 知识库 RAG 现行口径
- `POST /api/kb/search` 已升级为片段级召回后折叠文章结果；返回结构仍兼容原有 `items`，并会额外携带可选字段 `matchSnippet`、`matchHeading`、`matchScore`。
- `POST /api/kb/ask` 已改为基于 TopK 知识片段组装上下文；`citations` 除 `articleId/title/version` 外，还可能返回 `snippet`、`heading`、`chunkIndex`。
- 创建、更新、回滚、`/api/kb/import/docs` 导入成功后，后端会自动重建当前版本的知识片段索引，无需额外手工触发。
- 片段切分参数可通过环境变量调优：`KB_RAG_CHUNK_SIZE`、`KB_RAG_CHUNK_OVERLAP`。
