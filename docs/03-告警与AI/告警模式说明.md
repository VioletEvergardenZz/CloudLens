...
> 适用：`go-watch-file` 的告警决策模块（日志输入源 → 规则匹配 → 决策记录/通知）。
> 规则由控制台维护，并持久化到 `config.runtime.yaml` 的 `alert_rules`。

## 1. 功能概览
- 轮询日志文件或跟随 Docker 容器日志，按规则匹配日志行，输出告警决策。
...
alert_log_paths: "/var/log/app/error.log docker://service/api?project=myapp"
...
- `alert_log_paths`：告警输入源列表，支持逗号/分号/空白/中文“，/；”分隔。
...
- Docker 源支持：
  - `docker://container/<容器名或ID前缀>`
  - `docker://service/<服务名>?project=<compose项目>`
  - `docker://compose/<项目>/<服务>`
- Docker 日志会镜像到 `logs/alert-sources/docker/`，用于继续复用 AI 分析、全文检索与知识库链路。
...
- 若当前使用 Docker 源，优先从 `logs/alert-sources/docker/` 选择对应镜像日志。
...
- `POST /api/ai/log-summary`：触发 AI 日志分析（需开启 `ai_enabled` 且配置 `AI_*`）。
  ...
  - `path` 白名单：位于 `watch_dir` 下，或命中 `alert_log_paths` 中的路径；当配置 Docker 源时，也允许 `logs/alert-sources/docker/` 下的镜像日志。
